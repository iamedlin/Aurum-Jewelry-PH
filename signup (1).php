...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
 ...
 $conn = new mysqli("localhost", "u663344503_221024", "Database_3", "u663344503_users");
    ...
    if (move_uploaded_file($_FILES["profile"]["tmp_name"], $targetFile)) {
        $stmt = $conn->prepare("INSERT INTO user_info (name, email, contact, address, profile, password) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssss", $name, $email, $contact, $address, $targetFile, $password);
...
<head>
    ...
    <title>Sign Up - Aurum Jewelry PH</title>
    <style>
        body { 
...
<input type="text" name="contact" placeholder="Your contact number" required pattern="[0-9+]{10,15}" title="Enter a valid contact number" />
